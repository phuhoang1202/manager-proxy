...
const DropdownUser = () => {
    ...
    const logout = () => {
        ...
        // handle logic affter logout
        localStorage.setItem(CONSTANT.TRANSFER_INFO, JSON.stringify(null));
        localStorage.setItem(CONSTANT.USERLOGIN, JSON.stringify(null));
        dispatch(reset());
    }

    return (
        <ClickOutside className="relative" onClick={() => setDropdownOpen(false)}>
            <Link to="#" className="flex items-center gap-4" onClick={() => setDropdownOpen(!dropdownOpen)}>
                <span className="hidden text-right lg:block">
                    <span className="block text-sm font-medium text-black dark:text-white">
                        {userLogin?.username}
                    </span>
                    <span className="block text-xs">{userLogin?.email}</span>
                </span>

                <span className="h-12 w-12 rounded-full overflow-hidden">
                    <img src={UserOne} alt="User" srcSet='https://media.dolenglish.vn/PUBLIC/MEDIA/2b2f1391-7dcd-4d41-b1eb-2273c8cd00de.jpg' />
                </span>
